...
var declist = [{
        ...
        chiness: {
            title: "ap1 ",
            dec: "南海明珠,佛教福地。有“百岛之市”美誉的珠海，1980年经国务院批准成为中国五个经济特区之一。据2010年普查，珠海陆地面积1701平方千米，海岸线长604公里，大小岛屿146个。 "
        },
        english: {
            title: "Pearlof the South ChinaSea",
            dec: "Zhuhai,with a reputation of ‘City of Islands’，was approved by the State Council to be one of the Five Economic Zones in 1980.According to the census statistics of 2010，the land area of Zhuhai reaches 1,701 square kilometers,its coastline,604 kilometers and its islands ,large or small,146."
        }
    },
    {
        id: "ap2",
    ...
    },
    {
        ...
        english: {
            ...
            dec: "It is named so as it is placed before the Buddha statue symbolizing brightness through day and night.It is a manifestation of the compassion to lead people to the bright in an ever constant and progressing manner."
        }
    },
    {
        id: "ap4",
...
import TG_ScenicSpot from "../../static/json/TG_ScenicSpot.js";
/*处理geojsonpoi*/
var poiLayer = {};
geojsonObject.features.map(item => {
    ...
    data.title = item.properties.Name;
    data.id = item.id;
...
var sightLayer = [];
TG_ScenicSpot.features.map(item => {
    var type = item.properties.type;
...
export default {
    ...
    getSightMessageById({ commit, state }, payload) {
        return new Promise((resolve, reject) => {
            ...
            if (acitveItem.length && activeDec.length) {
